<template>
  <UContainer>
    ...
    <UTable
      :rows="data"
      :columns="[{ key: 'text', label: 'Comment' }]"
    >
      <template #text-data="{ row }">
        <span class="flex flex-row items-center gap-2">
          <span
            class="rounded-full h-2 w-2"
            :class="{ 'bg-green-400': row.isCurrent }"
            title="My comment"
          >
            <span
              v-if="row.isCurrent"
          ...
          <UButton
            v-if="row.isCurrent || userData?.userRole === 'admin'"
    ...
    >
      <UFormGroup
        label="Comments on the conference"
        name="text"
      >
...
<script setup lang="ts">
  ...
  const { isLoggedIn, logout } = useEdgeDbIdentity()
